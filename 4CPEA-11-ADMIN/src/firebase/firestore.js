...
export const subscribeDailyPassengerCount = (callback) => {
  ...
  const txUnsubscribe = onSnapshot(txQuery, (snapshot) => {
    ...
    tripTransactionsByUser.forEach((transactions, userId) => {
      if (!userStatuses.has(userId)) return
...
/**
 * Subscribe to daily terminal activity (boarding and alighting) from today's trip transactions
 * Same data source as revenue - counts completed trips by origin (boarding) and destination (alighting)
 * @param {Function} callback - Callback function that receives terminal data array
 * @returns {Function} Unsubscribe function
 */
export const subscribeDailyTerminalActivity = (callback) => {
  const { start, end } = getManilaDayRange()
  const startTimestamp = Timestamp.fromDate(start)
  const endTimestamp = Timestamp.fromDate(end)
  ...
  return onSnapshot(q, (snapshot) => {
    ...
    snapshot.forEach((docSnap) => {
      ...
      const from = data.route?.from ?? data.origin
      const to = data.route?.to ?? data.destination

      const fromId = from != null ? (typeof from === 'number' ? from : parseInt(from, 10)) : null
      const toId = to != null ? (typeof to === 'number' ? to : parseInt(to, 10)) : null

      if (fromId != null && terminalActivity[fromId]) {
        terminalActivity[fromId].boarding += 1
      }
      if (toId != null && terminalActivity[toId]) {
        terminalActivity[toId].alighting += 1
      }
    })
    ...
    console.error('Error listening to daily terminal activity:', error.message)
    callback([1, 2, 3, 4].map((id) => ({
      id,
      name: `Terminal ${id}`,
      boarding: 0,
      alighting: 0,
...
export const getHistoricalData = async (startDate, endDate) => {
  const emptyResult = {
    summary: { totalPassengers: 0, totalRevenue: 0, totalExpenses: 0, profit: 0 },
  ...
  try {
    ...
    if (isNaN(startDateObj.getTime()) || isNaN(endDateObj.getTime())) {
      console.warn('Invalid date range for report')
      return emptyResult
    }
    ...
    try {
      ...
      txSnapshot = await getDocs(txQuery)
    } catch (txError) {
      if (txError?.code === 'failed-precondition' && txError?.message?.includes('index')) {
        console.warn('Firestore index missing for transactions query. Create index on: type (ASC), timestamp (ASC). Report will show empty transaction data.')
        txSnapshot = { forEach: () => {}, empty: true }
      } else {
        throw txError
      }
    }
    ...
    try {
      ...
      if (expError?.code === 'failed-precondition' && expError?.message?.includes('index')) {
        console.warn('Firestore index missing for expenses query. Report will show empty expense data.')
    ...
    if (!txSnapshot.empty) {
      txSnapshot.forEach((docSnap) => {
        ...
        const from = data.route?.from ?? data.origin
        const to = data.route?.to ?? data.destination
        if (from != null && terminalActivity[from]) {
          terminalActivity[from].boarding += 1
        }
        if (to != null && terminalActivity[to]) {
          terminalActivity[to].alighting += 1
        }

        const ts = data.timestamp?.toDate?.() ?? data.timestamp
        const timestamp = ts instanceof Date ? ts : (ts ? new Date(ts) : new Date(0))
        ...
        if (data.userId) passengerSet.add(data.userId)
      })
    }
    ...
    expensesSnapshot.forEach((docSnap) => {
      ...
      const ts = data.createdAt?.toDate?.() ?? data.createdAt
      const createdAt = ts instanceof Date ? ts : (ts ? new Date(ts) : new Date(0))
      expenses.push({
        id: docSnap.id,
        amount,
